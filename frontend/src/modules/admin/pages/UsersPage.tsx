...
import { Coins, Edit3, LockKeyhole, Search, Trash2, UserPlus, X } from 'lucide-react';
...
type Modal = { type: 'create' } | { type: 'edit' | 'balance'; user: User } | null;
...
export function UsersPage() {
  ...
  const [balanceForm, setBalanceForm] = useState({ amount: 100000, token_amount: 1000000, type: 'credit', description: 'Admin credit' });
  ...
  return <div className="page">
    ...
    <article className="card table-card"><div className="table-section-heading"><div><p className="eyebrow">CLIENT DIRECTORY</p><h3>Danh sach tai khoan</h3></div><span>{filtered.length} ket qua</span></div>{filtered.length ? <div className="table-wrap"><table><thead><tr><th>Khach hang</th><th>So du</th><th>Token quota</th><th>Model da cap</th><th>Trang thai</th><th>Thao tac</th></tr></thead><tbody>{pagination.paginatedItems.map((user) => <tr key={user.id}>
      ...
      <td><b>{formatVnd(user.balance)}</b></td><td>{formatNumber(user.tokenUsed)} / {formatNumber(user.tokenQuota)}</td>
      ...
      <td><div className="row-actions"><button className="icon-button" title="Sua" onClick={() => openEdit(user)}><Edit3/></button><button className="icon-button" title="Dieu chinh so du" onClick={() => { setBalanceForm({amount:100000,token_amount:1000000,type:'credit',description:'Admin credit'}); setModal({type:'balance',user}); }}><Coins/></button><button className="icon-button" title="Khoa/mo" onClick={() => toggleStatus.mutate(user.id)}><LockKeyhole/></button><button className="icon-button danger-text" title="Luu tru" onClick={() => confirm(`Luu tru tai khoan ${user.email}?`) && archive.mutate(user.id)}><Trash2/></button></div></td>
