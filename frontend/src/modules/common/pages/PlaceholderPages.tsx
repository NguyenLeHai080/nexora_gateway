...
interface Log { id: number; requestId: string; model: string; status: string; inputTokens: number; outputTokens: number; cost: number; latencyMs: number; createdAt: string }
...
export function LogsPage() {
  ...
  return <div className="page"><PageHeader eyebrow="OBSERVABILITY / EVENTS" title="Nhat ky su dung" description="Theo doi request, model, token, latency va chi phi chi tiet."/><article className="card table-card"><div className="table-section-heading"><div><p className="eyebrow">REQUEST LOG</p><h3>Hoat dong gateway</h3></div><span>{data.length} request</span></div>{data.length?<div className="table-wrap"><table><thead><tr><th>Thoi gian</th><th>Request ID</th><th>Model</th><th>Trang thai</th><th>Token I/O</th><th>Latency</th><th>Chi phi</th></tr></thead><tbody>{pagination.paginatedItems.map((item) => <tr key={item.id}><td>{item.createdAt}</td><td><code>{item.requestId}</code></td><td><b>{item.model}</b></td><td><Badge tone={item.status === 'success' ? 'success' : 'danger'}>{item.status === 'success' ? <CheckCircle2/> : <XCircle/>}{item.status.toUpperCase()}</Badge></td><td>{formatNumber(item.inputTokens)} / {formatNumber(item.outputTokens)}</td><td>{item.latencyMs} ms</td><td>{formatVnd(item.cost)}</td></tr>)}</tbody></table></div>:<TableEmpty message="Chua co request nao duoc ghi nhan."/>}<TablePagination page={pagination.page} pageSize={pagination.pageSize} totalItems={pagination.totalItems} totalPages={pagination.totalPages} onPageChange={pagination.setPage} onPageSizeChange={pagination.changePageSize}/></article></div>;
}
