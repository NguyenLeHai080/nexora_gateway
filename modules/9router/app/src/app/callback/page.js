...
function CallbackContent() {
  ...
  useEffect(() => {
    ...
    // When the dashboard is behind a local reverse proxy (for example
    // 9route.test) the OAuth provider still redirects to localhost, so the
    // popup and its opener have different origins. Allow only the explicitly
    // configured dashboard origin; never fall back to a wildcard target.
    const configuredBaseUrl = process.env.NEXT_PUBLIC_BASE_URL;
    if (configuredBaseUrl) {
      try {
        const configuredOrigin = new URL(configuredBaseUrl).origin;
        if (!expectedOrigins.includes(configuredOrigin)) {
          expectedOrigins.push(configuredOrigin);
        }
      } catch {
        // Ignore an invalid optional URL and retain the strict allowlist.
      }
    }
